...
function twice(num) {
  let firstHalf = Math.floor(String(num).slice(0, String(num).length / 2));
  let lastHalf = Math.floor(String(num).slice(String(num).length / 2));
  return firstHalf === lastHalf ? num : num * 2;
}
