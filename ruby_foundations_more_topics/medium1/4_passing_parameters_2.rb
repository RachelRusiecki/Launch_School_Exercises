...
# Can we do that without extracting items directly from the array based on their indices? We can use the splat operator(*) to do something like this:
...
# The method should yield the contents of the array to a block, which should assign your block parameters in such a way that it ignores the first two elements.
# Groups all remaining elements as a raptors array.

def bird_method(birds)
  yield(birds)
end

bird_method(%w(raven finch hawk eagle)) { |_, _, *raptors| p raptors }
